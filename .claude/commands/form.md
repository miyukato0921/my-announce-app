---
description: 新しいリソース（例：お知らせ、商品）の投稿フォームを追加するとき、既存のフォームにバリデーションや新しい入力項目を追加するパターンを使って実装するときに使う
---

## 追加できること

- テキスト・パスワード・textarea フィールドを持つ送信フォーム
- 送信中スピナー・二重送信防止ボタン
- エラーメッセージ・成功メッセージの表示
- パスワード表示トグル
- 送信後のフォームリセット
...
- `components/ui/button.tsx` — shadcn/ui の `Button` コンポーネント
- `components/ui/input.tsx` — shadcn/ui の `Input` コンポーネント
- `components/ui/label.tsx` — shadcn/ui の `Label` コンポーネント
- `components/ui/textarea.tsx` — shadcn/ui の `Textarea` コンポーネント
